...
> **Status**: Phase 1 MVP - Production Ready | **Updated**: 2026-09-16 (Spec E)
...
- weekly shopping list aggregated from the meal plan (compatible-unit + singular/plural merging), plus ad-hoc "Voglio preparare questo" additions from any recipe, independent of the weekly plan; a "Per reparto / Per ricetta" toggle groups the list by supermarket department (13-department taxonomy shared with the pantry, precedence chain pantry → user override → curated dictionary → "Altro", user-correctable via "Sposta in reparto…")
...
### 2026-09-15 — Shopping list grouped by supermarket department (Spec E)
- **Taxonomy** `PANTRY_CATEGORIES` (`lib/utils/pantry-utils.ts`) extended from 10 to 13 slugs (`surgelati`, `panetteria`, `altro` appended, `altro` an explicit fallback rather than an implicit unknown-slug bucket); the dispensa page's `itemsByCategory` memo now remaps unknown/historical slugs into `'altro'` and the duplicate "Altro" IIFE section was deleted
- **Classification** `lib/utils/ingredient-departments.ts`: precedence chain matched pantry entry's `categoryId` → user override → curated static dictionary (~290 Italian ingredient entries, `classifyIngredientDepartment`) → `'altro'` fallback. Stem collisions from the aggregator's stemmer (`pesca`/`pesce`, `grana`/`grano`) resolved explicitly in the seed and guarded by a "no collisions" test
- **Overrides** "Sposta in reparto…" persists to `users/{uid}.ingredientDepartmentOverrides` (`lib/firebase/department-overrides.ts` + `useDepartmentOverrides`), same user-doc-extension pattern as `familyProfile`/`adHocShoppingRecipes` — no new collection/rule/index, no debounce (rare, deliberate action, so it does not enter `useShoppingList`'s `flushAll()`)
- **View model** `lib/utils/shopping-departments.ts` (`buildDepartmentSections`, pure/tested): builds per-department sections in `PANTRY_CATEGORIES` order from the SAME visible items as the per-recipe view (pantry-owned items excluded upstream, same array both views consume) — plan, custom and ad-hoc rows, unchanged ids so check marks stay valid across views, no cross-block merge
- **UI**: `ShoppingViewToggle` ("Per reparto" default / "Per ricetta", per-device `localStorage`, not Firestore), `DepartmentSection` (color-dot header, never a side-stripe), `MoveToDepartmentSheet`; `ShoppingItemRow` gained `onMove?`, always visible, hidden only for rows whose department comes from the matched pantry entry (an override there would lose on precedence); `AddCustomItemSheet` gained an optional "Reparto" select that writes an override on submit
- **Spec D dependency**: `src/lib/utils/ingredient-matching.ts` was already on `develop` (merged same day) with `canonicalIngredientKey`, `matchIngredientToPantry`, `PantryItem.aliases` — Spec E's §4.0 "bring forward the minimal subset" phase was skipped entirely, nothing to move
- **Verified** (2026-09-15): `npx tsc --noEmit` exit 0 · `npm test` 257/257 · `npx next build --webpack` exit 0 (no `spawn EPERM`) · guided test: see "Guided testing tooling" below
- **Not touched**: `canonicalIngredientKey`/`toSlug`/stemmer, cross-block plan↔ad-hoc merge, AI endpoints, kcal in the shopping list (confirmed exclusion)
